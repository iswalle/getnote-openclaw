...
AI 会先读取真实知识库、文件夹和权限，再执行操作，不会根据名称猜 ID。列表默认只查普通知识库；书籍、客户档案和团队知识库会按目标类型显式查询，订阅列表不会混入自己创建的知识库。
...
| 日期 | 版本 | 新能力 | 适合怎么用 |
|------|------|--------|------------|
| 2026-09-24 | **v2.1.0** | 独立读取标记、发芽、章节时间线；查询文件能力并上传知识库；支持云 MCP 临时授权直传 | 配合 CLI 1.6.0，文件直接上传 OSS，无需二次 CLI 授权，入库后确认解析成功 |
| 2026-09-02 | **v2.0.4** | 知识库列表默认只查 `DEFAULT`，特殊类型按 scope 精确查询；订阅列表排除自有知识库 | 避免书籍库挤占分页，并准确找到真正订阅的知识库 |
| 2026-08-19 | **v2.0.3** | 统一安装、更新与诊断闭环，并与 CLI capabilities 2.2 对齐 | 用户只需表达“安装”或“更新”，AI 即可完成 CLI、领域 Skill 与独立 Skill 的正确分工和验收 |
