...
| 能力 | 说明 |
|------|------|
| **📎 一键保存链接** | 明确要求保存到得到大脑后，云服务抓取公开网页并生成摘要 |
| **🖼 图片秒变笔记** | 用户确认上传后，由得到大脑云服务识别和保存图片内容 |
...
| **📤 生成分享链接** | 用户明确确认后生成公开链接；任何获得链接的人都可能查看分享内容 |
...
> 👤 记到得到大脑：支付流程可以加一个进度条，用户等待时不焦虑
>
> 🤖 已记录，自动打上「产品优化」标签。

**听播客时有感触**

> 👤 把这个观点存到得到大脑：好的产品是让用户少做选择，不是多做选择
>
> 🤖 已记录。
...
> 👤 我想写篇关于做产品的文章，帮我在得到大脑里找找之前的相关想法
>
...
- API Key 只会发送到得到大脑官方接口 `https://openapi.biji.com`
- 保存和搜索时，相关文字、搜索词、知识库信息、链接、图片或录音内容会发送到得到大脑云服务处理
- 不要保存含登录态、临时签名、内网地址或敏感查询参数的链接；确需保存时先确认风险
- 删除笔记、删除标签和移出知识库前，AI 必须展示对象并获得明确确认
- 生成分享链接前，AI 必须提示该链接可能公开笔记内容并获得明确确认
- 普通对话中的「保存」「收藏」「搜一下」不会单独触发本 Skill；需要明确提到得到大脑、Get笔记或自己的笔记库
...
| 日期 | 版本 | 新能力 | 适合怎么用 |
|------|------|--------|------------|
| 2026-07-27 | **v1.9.1** | 收紧凭证发送地址，补充权限、隐私提示和高风险操作确认 | 更安全地授权、搜索、删除和公开分享笔记 |
