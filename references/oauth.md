...
> 🔐 **仅在明确意图下授权**：只有用户明确要求连接得到大脑，或明确要求执行得到大脑操作但尚未连接时，才能提出授权。发起前说明授权会让 Skill 按用户勾选的权限访问得到大脑，并且笔记内容、搜索词、知识库信息及用户主动保存的链接/图片会发送到得到大脑云服务处理。先询问“是否现在连接得到大脑？”，用户确认后再创建设备码；不得在无关对话中自动弹出授权。

---

## 触发条件

仅在用户已经明确要求操作得到大脑时检查 `$GETNOTE_API_KEY`。若不存在，说明授权用途并询问用户是否现在连接；用户确认后引导完成授权，配置成功后继续原本的得到大脑任务。
